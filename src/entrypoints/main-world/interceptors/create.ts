...
export function handleCreatePromptResponse(response: any, url: string) {
    if (response.status === 200) {
      console.log(
        'Better Sidebar for Google AI Studio: Intercepted CreatePrompt'
      );
      
      // 1. Dispatch legacy/specific event for Folder creation logic
      ...
      // [ "prompts/ID", null, null, null, ["Title", ...], ... ]
      // json[0] -> "prompts/ID"
      // json[4][0] -> Title
      // json[4][2] -> Metadata
      ...
      if (Array.isArray(json)) {
        ...
        if (typeof promptPath === 'string') {
          ...
          if (id) {
            console.log(
              `Better Sidebar for Google AI Studio: Detected new prompt creation: ${id}`
            );
            globalThis.dispatchEvent(
              new CustomEvent('AI_STUDIO_PROMPT_CREATE', {
                detail: {
                  ...
                  created_at: createdAt || Math.floor(Date.now() / 1000), // Use seconds, not milliseconds
                  originalUrl: url,
                  type,
                },
              })
      ...
      // 2. Dispatch generic response event for Data Persistence (saving messages)
