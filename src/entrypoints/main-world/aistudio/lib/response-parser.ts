/**
 * AI Studio specific response parser
 * Extracts conversation and prompt data from AI Studio API responses.
 */
...
export function parsePromptType(
  ...
  try {
    if (Array.isArray(properties)) {
      const promptTypeEntry = properties.find(
        (prop: any) => Array.isArray(prop) && prop[0] === 'promptType'
      );
...
function findHistoryRecursive(node: any, depth = 0): any[] | null {
  ...
  const childrenAreTurns = firstFew.some(
    (child) => Array.isArray(child) && isTurn(child)
  );
...
export function parseConversation(json: any): ParsedConversation | null {
  try {
    ...
    if (
      ...
      json[0][0].startsWith('prompts/')
    ) {
      root = json[0];
    } else if (
      Array.isArray(json) &&
      typeof json[0] === 'string' &&
      json[0].startsWith('prompts/')
    ) {
      root = json;
    } else {
      if (
    ...
    if (typeof root[0] !== 'string' || !root[0].startsWith('prompts/'))
      return null;
