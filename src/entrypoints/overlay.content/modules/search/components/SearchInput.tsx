...
export const SearchInput = () => {
  ...
  const {
    ...
    options,
    isSearching
  } = useAppStore(state => state.ui.search);
  ...
  return (
    <div className="flex flex-col gap-2 p-4 border-b">
      <div className="relative flex items-center">
        ...
        <div className="absolute right-1 flex items-center gap-0.5">
          <Button
            variant="ghost"
            size="icon"
            className={cn("h-7 w-7", options.caseSensitive && "bg-accent text-accent-foreground")}
            onClick={() => {
          ...
          <Button
            variant="ghost"
            size="icon"
            className={cn("h-7 w-7", options.wholeWord && "bg-accent text-accent-foreground")}
            onClick={() => {
      ...
      <div
        className="flex items-center justify-between text-muted-foreground hover:text-foreground select-none"
      >
        <div className={cn("flex items-center gap-1", !options.showOptions && "invisible")}>
          <span className="text-xs">{t('search.foldersToInclude')}</span>
          <SimpleTooltip content={t('search.foldersIncludeTooltip')}>
            <HelpCircle className="h-3 w-3 text-muted-foreground cursor-help" />
          </SimpleTooltip>
        </div>
        <MoreHorizontal className="h-4 w-4 cursor-pointer" onClick={() => setSearchOptions({ showOptions: !options.showOptions })} />
      </div>
        ...
        <div className="flex flex-col gap-2 animate-in slide-in-from-top-2 fade-in duration-200">
          ...
          <div className="flex flex-col gap-1">
            <div className="flex items-center gap-1">
              <label className="text-xs text-muted-foreground">{t('search.foldersToExclude')}</label>
          ...
          <div className="flex flex-col gap-1 pt-1">
             <label className="text-xs text-muted-foreground">{t('search.filterByRole')}</label>
