...
export const FavoritesList = ({ searchQuery = '', selectedTags = [], typeFilter = 'all' }: FavoritesListProps) => {
  const { t } = useI18n();
  const { favorites, conversations, conversationTags, toggleFavorite } = useAppStore();
  ...
  const favoriteItems = useMemo(() => {
    ...
    if (selectedTags.length > 0) {
      filteredItems = filteredItems.filter(item => {
        ...
        const itemTags = new Set(conversationTags
  ...
  }, [favorites, conversations, conversationTags, searchQuery, selectedTags, typeFilter]);
         ...
         {favoriteItems.map(item => {
           ...
           return (
            <div 
              ...
              className={cn(
                "group flex items-center gap-2 px-3 py-1.5 cursor-pointer text-sm text-foreground transition-colors",
                isSelected ? "bg-accent text-accent-foreground" : "hover:bg-accent/50"
              )}
              onClick={() => handleOpen(item.external_url)}
            >
               {isImage ? (
                   <ImageIcon className={cn("w-4 h-4 shrink-0", isSelected ? "text-accent-foreground" : "text-muted-foreground")} />
               ) : (
                   <MessageSquare className={cn("w-4 h-4 shrink-0", isSelected ? "text-accent-foreground" : "text-muted-foreground")} />
               )}
               ...
               <SimpleTooltip content={t('tooltip.removeFromFavorites')}>
                 <Button 
                   ...
                   className={cn(
                     ...
                     isSelected && "text-accent-foreground hover:text-accent-foreground hover:bg-accent-foreground/10"
                   )}
                   onClick={(e) => {
                     e.stopPropagation();
                 ...
                 >
                   <Star className={cn("h-3.5 w-3.5 fill-current text-yellow-500 fill-yellow-500")} />
                 </Button>
               </SimpleTooltip>
            </div>
           );
