...
export const TimelineView = forwardRef<ArboristTreeHandle, TimelineViewProps>(({ onSelect }, ref) => {
  const {
    treeRef,
    containerRef,
    dimensions,
  ...
  const { conversationTags, ui, favorites, setExplorerBatchSelection } = useAppStore();
  ...
  const data = useMemo(() => {
    ...
    console.log('TimelineView: Recalculating data. Conversations:', filteredConversations.length);
    const groups = new Map<string, NodeData>();
    const predefinedKeys = ['today', 'yesterday', 'previous7Days', 'previous30Days'];
    ...
    const getDateGroupInfo = (timestamp: number) => {
      ...
      if (diffDays === 0) return { key: 'today', label: t('timeline.today'), sortValue: Infinity };
      if (diffDays === 1) return { key: 'yesterday', label: t('timeline.yesterday'), sortValue: Infinity - 1 };
      if (diffDays <= 7) return { key: 'previous7Days', label: t('timeline.previous7Days'), sortValue: Infinity - 2 };
      if (diffDays <= 30) return { key: 'previous30Days', label: t('timeline.previous30Days'), sortValue: Infinity - 3 };

      return {
        key: `month-${date.getFullYear()}-${date.getMonth()}`,
        label: date.toLocaleString(currentLanguage, { month: 'long', year: 'numeric' }),
    ...
    filteredConversations.forEach(c => {
      ...
      const ts = (timestamp > 0 && timestamp < 100000000000) ? timestamp * 1000 : timestamp;
    ...
    return sortedGroups;
  }, [conversations, tagFilter.selected, conversationTags, typeFilter, onlyFavorites, favorites, t, currentLanguage]);
